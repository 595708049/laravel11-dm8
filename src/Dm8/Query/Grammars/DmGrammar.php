...
class DmGrammar extends Grammar
{
    ...
    /**
     * Determine if the database connection uses lowercase table and column names.
     *
     * @var bool
     */
     ...
     * @return bool
     */
    protected function isPaginationable(Builder $query, array $components): bool
    {
     ...
     * @return string
     */
    public function wrap(mixed $value, bool $prefixAlias = false): string
    {
    ...
    public function compileInsert(Builder $query, $values)
    {
        ...
        // For single record insertion, use standard parameter binding
        if (count($values) === 1) {
            $parameters = $this->parameterize(reset($values));
            return "insert into $table ($columns) values ($parameters)";
        }

        // For batch insertion, use a safer approach that maintains parameter binding
        $parameterGroups = [];
